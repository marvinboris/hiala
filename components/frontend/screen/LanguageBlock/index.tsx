...
export default function LanguageBlock({ language, selected, select }: LanguageBlockProps) {
    return <div onClick={() => select(language.abbr)} className={classNames("rounded-[21.6981px] md:rounded-[25px] w-[106px] flex flex-col cursor-pointer items-center p-[13px]", selected ? 'bg-white border relative border-green shadow-lg shadow-primary/20' : 'bg-secondary-100')}>
        ...
        <div className={classNames(selected ? 'text-white bg-green' : 'text-secondary-700 bg-primary/20', "w-[66px] md:w-[76px] text-center py-[5.5px] font-medium text-[10px] md:text-xs rounded-3xl")}>{language.name}</div>
    </div>
}